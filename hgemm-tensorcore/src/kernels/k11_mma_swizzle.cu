...
const int block_j_swizzle_factor = 8;
...
__global__ static void kernel11(GEMM_FUNC_SIGNITURE) {
...
void gemm_11_mma_swizzle(GEMM_FUNC_SIGNITURE) {
    ...
    if ((float)alpha != 1 || (float)beta != 0) {
        std::cout << "gemm_11_mma_swizzle kernel only supports computing C=A*B (alpha=1, beta=0)\n";
        exit(-1);
    }
