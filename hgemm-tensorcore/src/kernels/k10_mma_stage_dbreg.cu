...
__global__ static void kernel10(GEMM_FUNC_SIGNITURE) {
...
void gemm_10_mma_stage_dbreg(GEMM_FUNC_SIGNITURE) {
    ...
    if ((float)alpha != 1 || (float)beta != 0) {
        std::cout << "gemm_10_mma_stage_dbreg kernel only supports computing C=A*B (alpha=1, beta=0)\n";
        exit(-1);
    }
    ...
    cudaFuncSetAttribute(kernel10,
                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                         smem_size);

    dim3 gridDim(M / bM, N / bN);
    dim3 blockDim(32 * num_wM * num_wN);
    kernel10<<<gridDim, blockDim, smem_size>>>(GEMM_FUNC_PARAM);
}