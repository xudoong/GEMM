...
__global__ static void kernel09(GEMM_FUNC_SIGNITURE) {
    ...
    for (int stage = 0; stage < num_stages - 1; stage++) {
        load_shared_AB(sA(stage), A + stage * bK, K, tid, bM, offset_ij, offset_k);
        load_shared_AB(sB(stage), B + stage * bK, K, tid, bN, offset_ij, offset_k);
        copy_async_commit();
    }
    ...
    for (int stage = num_stages - 1; stage < K / bK; stage++) {
        ...
        compute(lane, wi, wj, sA(comp_stage), sB(comp_stage), rA, rB, rC);
        load_shared_AB(sA(copy_stage), A + stage * bK, K, tid, bM, offset_ij, offset_k);
        load_shared_AB(sB(copy_stage), B + stage * bK, K, tid, bN, offset_ij, offset_k);
        copy_async_commit();
        copy_async_wait_and_syncthreads<num_stages - 2>();
    ...
    if (num_stages >= 2) {
        ...
        compute(lane, wi, wj, sA(stage), sB(stage), rA, rB, rC);
...
void gemm_09_mma_stage(GEMM_FUNC_SIGNITURE) {
    ...
    if ((float)alpha != 1 || (float)beta != 0) {
        std::cout << "gemm_09_mma_stage kernel only supports computing C=A*B (alpha=1, beta=0)\n";
        exit(-1);
    }
    ...
    cudaFuncSetAttribute(kernel09,
                         cudaFuncAttributeMaxDynamicSharedMemorySize,
                         smem_size);

    dim3 gridDim(M / bM, N / bN);
    dim3 blockDim(32 * num_wM * num_wN);
    kernel09<<<gridDim, blockDim, smem_size>>>(GEMM_FUNC_PARAM);
}