...
bool verify_matrix(half *matRef, half *matOut, int N)
{
    ...
    for (i = 0; i < N; i++)
    {
        diff = fabs(float(matRef[i]) - float(matOut[i]));
        ...
        {
            printf("Divergence! Should %f, Is %f (diff %f, rerr %f) at %d\n",
...
void run_kernel(int kernel_num, cublasHandle_t handle, GEMM_FUNC_SIGNITURE)
{
    switch (kernel_num)
    {
       ...
       case 6:
            ...
            break;
       case 7:
            gemm_07_mma_padding(GEMM_FUNC_PARAM);
            break;
       case 8:
            gemm_08_mma_permute(GEMM_FUNC_PARAM);
            break;
       case 9:
