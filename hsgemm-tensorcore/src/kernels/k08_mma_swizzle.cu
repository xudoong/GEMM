...
const int num_bN_inner = 8; // wrap in cta to blocks(N-dim) mappings
...
__global__ static void kernel08(GEMM_FUNC_SIGNITURE) {
    ...
    const int num_bM = M / bM;
    const int num_bN = N / bN;


    const int bi = (blockIdx.z % 2) ? blockIdx.y : (num_bM - 1 - blockIdx.y);
    const int bj = blockIdx.z * num_bN_inner + blockIdx.x;
...
void gemm_08_mma_swizzle(GEMM_FUNC_SIGNITURE) {
    ...
    assert(bK2 <= 8);
    if (alpha != 1 || beta != 0) {
        std::cout << "gemm_08_mma_swizzle kernel only supports computing C=A*B (alpha=1, beta=0)\n";
        exit(-1);
    }

    dim3 gridDim(num_bN_inner, M / bM, CEIL_DIV(N / bN, num_bN_inner));
    dim3 blockDim(32, num_wM, num_wN);
    kernel08<<<gridDim, blockDim>>>(GEMM_FUNC_PARAM);
}